...
void CEventMgr::tick()
{
	...
	for (size_t i = 0; i < m_vecEvent.size(); ++i)
	{
		...
		case EVENT_TYPE::LEVEL_CHANGE:
		{
			...
			LEVEL_TYPE eNextLevel = (LEVEL_TYPE)m_vecEvent[i].wParam;//level type으로 바꾸어주고
			CLevelMgr::GetInst()->ChangeLevel(eNextLevel);//LVMG에서 change level로 레벨 변경해준다 
		}
		break;
		case EVENT_TYPE::CHANGE_AI_STATE:
		{
			// wParam : AI Component Adress, lParam : Next State Name
			CSAI* pAI = (CSAI*)m_vecEvent[i].wParam;//주소값 얻어오고
